...
pub async fn run() -> Result<()> {
  ...
  match cli.command {
    ...
    Command::Jobsets { command } => jobsets(&api, cli.json, command).await,
    Command::Builds { command } => builds(&api, cli.json, command).await,
    Command::Queue => queue(&api, cli.json).await,
    Command::Logs { command } => logs(&api, cli.json, command).await,
    Command::Search { query, limit } => {
      search(&api, cli.json, &query, limit).await
    },
    Command::Channels { command } => channels(&api, cli.json, command).await,
    Command::News { command } => news(&api, cli.json, command).await,
    Command::Admin { command } => admin(&api, cli.json, command).await,
...
async fn jobsets(
  ...
  match command {
    ...
    JobsetCommand::Create {
      ...
      let mut body = Map::new();
      body.insert("name".to_string(), Value::String(name));
      body.insert("nix_expression".to_string(), Value::String(nix_expression));
...
async fn queue(api: &ApiClient, json_output: bool) -> Result<()> {
  let response = api.get("api/v1/operator/overview", false).await?;
  if json_output {
    return print_json(&response);
  }
...
async fn channels(
  ...
  match command {
    ChannelCommand::List => {
      let response = api.get("api/v1/channels", false).await?;
      if json_output {
        return print_json(&response);
      }
      print_table(
    ...
    ChannelCommand::Show { id } => {
      let response = api.get(&format!("api/v1/channels/{id}"), false).await?;
      if json_output {
        return print_json(&response);
      }
      print_table(&["ID", "Name", "Project", "Jobset", "Evaluation"], &[vec![
        field(&response, "id"),
        field(&response, "name"),
    ...
    ChannelCommand::Delete { id } => {
      let response = api.delete(&format!("api/v1/channels/{id}"), true).await?;
      if json_output {
        return print_json(&response);
      }
...
async fn news(
  ...
  match command {
    NewsCommand::List => {
      let response = api.get("api/v1/news", false).await?;
      if json_output {
        return print_json(&response);
      }
      print_table(
    ...
    NewsCommand::Delete { id } => {
      let response = api.delete(&format!("api/v1/news/{id}"), true).await?;
      if json_output {
        return print_json(&response);
      }
...
async fn admin(
  ...
  match command {
    AdminCommand::Status => status(api, json_output).await,
    AdminCommand::ApiKeys { command } => {
      api_keys(api, json_output, command).await
    },
    AdminCommand::Users { command } => users(api, json_output, command).await,
    AdminCommand::Builders { command } => {
      builders(api, json_output, command).await
    },
    AdminCommand::Notifications { command } => {
      notifications(api, json_output, command).await
    },
    AdminCommand::PinnedOutputs { command } => {
      pinned_outputs(api, json_output, command).await
    },
    AdminCommand::Audit { limit, offset } => {
      audit(api, json_output, limit, offset).await
    },
    AdminCommand::Config { command } => config(api, json_output, command).await,
  }
}
