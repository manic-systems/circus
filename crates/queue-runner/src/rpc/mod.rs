...
//! the bootstrap `Runner` capability accepts `register`, authenticates it
//! (bearer token or OIDC), persists the agent row in `builder_sessions`, and
//! inserts a live [`pool::AgentMeta`] into the pool. The pool is read by the
