...
/// Serializes in "pure" format, omitting impure fields when they have default
/// values. Used for content-addressed paths.
...
pub struct UnkeyedValidPathInfo {
  ...
  #[cfg_attr(
    ...
    proptest(strategy = "arb_signatures()")
  )]
...
fn deserialize_info<'de, D>(
  deserializer: D,
) -> Result<UnkeyedValidPathInfo, D::Error>
where
...
mod tests {
  ...
  fn test_fingerprint_path() {
    ...
    let store_path = StorePath::from_bytes(
      b"syd87l2rxw8cbsxmxl853h0r6pdwhwjr-curl-7.82.0-bin",
    )
    .unwrap();
    let nar_hash: NarHash =
    ...
    references.insert(
      StorePath::from_bytes(b"0jqd0rlxzra1rs38rdxl43yh6rxchgc6-curl-7.82.0")
        .unwrap(),
    );
    let fingerprint =
      fingerprint_path(&store_dir, &store_path, &nar_hash, 196040, &references);
