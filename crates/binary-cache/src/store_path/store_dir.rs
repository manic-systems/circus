...
impl StoreDir {
  ...
  /// This can fail if the path contains non-UTF-8 characters and therefore
  ...
  /// Strip the store directory prefix from a path string, returning the base
  /// name.
  ///
  /// Given a full store path like `/nix/store/hash-name`, returns `hash-name`.
  pub fn strip_prefix<'s>(
    &self,
    s: &'s str,
  ) -> Result<&'s str, StorePathError> {
    let path = Path::new(s);
    if !path.is_absolute() {
