...
use super::{FromStoreDirStr, StoreDir, StoreDirDisplay};
...
impl StorePath {
  pub fn from_bytes(buf: &[u8]) -> Result<Self, StorePathError> {
    ...
    let hash =
      StorePathHash::decode_digest(&buf[..STORE_PATH_HASH_ENCODED_SIZE])?;
    let name = buf[(STORE_PATH_HASH_ENCODED_SIZE + 1)..]
      .try_into()
      .map_err(|err| {
        StorePathError::adjust_index(
          STORE_PATH_HASH_ENCODED_SIZE as u8 + 1,
          err,
        )
      })?;
  ...
  pub fn serialize_as_base_path<S>(
    &self,
    serializer: S,
  ) -> Result<S::Ok, S::Error>
  where
  ...
  /// Deserialize from base path (just the hash-name part, without store
  /// directory)
  pub fn deserialize_from_base_path<'de, D>(
    deserializer: D,
  ) -> Result<Self, D::Error>
  where
    D: Deserializer<'de>,
...
const STORE_PATH_HASH_ENCODED_SIZE: usize =
  base32::encode_len(STORE_PATH_HASH_SIZE);
...
pub fn into_name<V: AsRef<[u8]>>(s: &V) -> Result<&str, StorePathNameError> {
  ...
  // SAFETY: We checked above that it is a subset of ASCII, which guarantees
  // valid UTF-8.
...
impl StorePathError {
  fn adjust_index(prefix: u8, other: StorePathNameError) -> StorePathError {
    match other {
      StorePathNameError::Symbol(old, ch) => {
        StorePathError::Symbol(prefix + old, ch)
      },
...
pub mod proptest {
  ...
  pub fn arb_store_path(
    max: u8,
    extension: Option<String>,
  ) -> impl Strategy<Value = StorePath> {
...
mod unittests {
  ...
  #[case::too_short(
    "zzcfcjwxkn4cf1nh8dh521vffyq2417",
    StorePathError::HashLength
  )]
  #[case::too_long(
    "zzcfcjwxkn4cf1nh8dh521vffyq24179a",
    StorePathError::HashLength
  )]
  #[case::invalid_symbol(
    "zzcfcjwxkn4|f1nh8dh521vffyq24179",
  ...
  fn name_ok(#[case] case: &str) {
    ...
    let name2: StorePathName =
      case.as_bytes().try_into().expect("parses bytes");
  ...
  fn store_path_error(
    #[case] path: &str,
    #[case] expected: ParseStorePathError,
  ) {
    assert_eq!(
  ...
  #[case(
    "/nix/store/ywrs8hr8fa4244bpdxi88bd87qxqgmy0-.-_?\
     +=abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSSTUVWXYZ",
    "ywrs8hr8fa4244bpdxi88bd87qxqgmy0-.-_?\
     +=abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSSTUVWXYZ"
  )]
  #[test]
  fn from_store_dir_str(
    #[case] store_path: &str,
    #[case] base_path: StorePath,
  ) {
    let store = StoreDir::default();
    let path: StorePath =
      store.parse(store_path).expect("Can parse store path");
  ...
  #[case::empty(
    ...
    ParseStorePathError::new(
      "",
      StorePathError::NonAbsolute(PathBuf::from(""))
    )
  )]
  ...
  fn to_absolute_path() {
    ...
    let path: StorePath =
      "ywrs8hr8fa4244bpdxi88bd87qxqgmy0-app-home".parse().unwrap();
