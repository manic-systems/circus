...
use harmonia_utils_hash::{
  ...
  Sha256,
  UnknownAlgorithm,
  fmt::{NonSRI, ParseHashError, ParseHashErrorKind},
};
    ...
    D: Deserializer<'de>,
  {
    ...
    Ok(match raw.method {
      ...
      ContentAddressMethod::Flat => {
        ContentAddressMethodAlgorithm::Flat(raw.hash_algo)
      },
...
impl ContentAddressMethodAlgorithm {
  ...
  pub fn method(&self) -> ContentAddressMethod {
    match self {
      ...
      ContentAddressMethodAlgorithm::NixArchive(_) => {
        ContentAddressMethod::NixArchive
      },
    }
  }
...
pub enum ParseContentAddressError {
  ...
  #[error(
    "'{0}' is not a content address because it is not in the form '<fixed | \
     text>:<rest>'"
  )]
  InvalidForm(String),
}
...
mod unittests {
  ...
  #[should_panic = "'test:sha256:\
                    1b8m03r63zqhnjf7l5wnldhh7c134ap5vpj0850ymkq1iyzicy5' is \
                    not a content address because it is not in the form \
                    '<fixed | text>:<rest>'"]
  #[case("test:sha256:1b8m03r63zqhnjf7l5wnldhh7c134ap5vpj0850ymkq1iyzicy5")]
  #[should_panic = "'test-12345' is not a content address because it is not in \
                    the form '<fixed | text>:<rest>'"]
  #[case("test-12345")]
  #[should_panic = "content address text hash \
                    'sha1:kpcd173cq987hw957sx6m0868wv3x6d9' should have type \
                    'sha256' but got 'sha1'"]
  ...
  // fn content_address_method_parse(#[case] method: ContentAddressMethod,
  // #[case] value: &str) { assert_eq!(method.to_string(), value);
