...
// SAFETY: The mapped memory is read-only and backed by immutable nix store
// files. The mapping lives as long as the MappedFile struct (unmapped on Drop).
...
impl MappedFile {
  /// Memory-map a file for reading. Returns an empty mapping for zero-length
  /// files.
  pub fn open(path: &Path, size: u64) -> io::Result<Self> {
    ...
    let len = usize::try_from(size).map_err(|_| {
      io::Error::other("file too large to mmap on this platform")
    })?;
  ...
  pub fn as_slice(&self) -> &[u8] {
    ...
    // SAFETY: ptr is valid for len bytes (from mmap), read-only, and lives
    // until drop.
