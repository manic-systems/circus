...
use super::{
  NarEvent,
  read_nar::{Inner, InnerState, NodeType},
};
use crate::{ByteString, padded_reader::PaddedReader};
  ...
  fn poll_next(
    ...
    match this.state.state {
      InnerState::ReadContents(
        NodeType::ExecutableFile | NodeType::File,
        ..,
      )
      | InnerState::ReadDir => {
...
pub async fn read_nar<R>(
  source: R,
) -> io::Result<crate::archive::test_data::TestNarEvents>
where
  R: AsyncRead + Unpin,
{
...
mod unittests {
  ...
  use super::*;
  use crate::archive::{
    read_nar::{TOK_DIR, TOK_ENTRY, TOK_ROOT, TOK_SYM},
    test_data,
    write_nar,
  };
