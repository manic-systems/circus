      ...
      RMatch::Mismatch(idx) => {
        ...
        match state {
          ...
          RLookup::Edge(edge_idx, read) => {
            ...
            let (left_edge_idx, len) =
              self.add_edge_list(node_idx, left_prefix);
    ...
    match state {
      RLookup::Node(node_idx) => {
        ...
        match node {
          ...
          RNode::Branch(..) => {
            panic!("invalid state to get value. must be leaf node")
          },
        }
      },
      RLookup::Edge(..) => panic!("invalid state to get value. must be node"),
    }
  }
