    ...
    match this.state {
      State::Content(rem) => {
        (
          ...
          rem
            .saturating_add(*this.trailer_size as u64)
            .try_into()
            .unwrap_or(usize::MAX),
          rem
            .saturating_add(aligned as u64)
            .try_into()
            .unwrap_or(usize::MAX),
        )
      },
    ...
    loop {
      match &mut this.state {
        State::Content(rem) => {
          ...
          // SAFETY: This is guaranteed to be the number of initialized (and
          // read) bytes due to the invariants provided by
          // `ReadBuf::filled`.
