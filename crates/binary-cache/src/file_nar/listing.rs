...
///
/// The resulting tree has the same JSON format as `nix nar ls --json
/// --recursive`.
pub async fn parse_nar_listing<R>(
  reader: R,
) -> std::io::Result<FileTree<NarFileInfo>>
where
  ...
  while let Some(event) = parser.next().await {
    ...
    match event {
      ...
      NarEvent::EndDirectory => {
        let (name, entries) = stack.pop().ok_or_else(|| {
          std::io::Error::other("EndDirectory without StartDirectory")
        })?;
...
fn target_str(target: &bytes::Bytes) -> std::io::Result<String> {
  std::str::from_utf8(target)
    .map(|s| s.to_owned())
    .map_err(|e| {
      std::io::Error::other(format!("non-UTF-8 symlink target: {e}"))
    })
}
