...
/// Calculate the length of a base64 encoded string for a given decoded byte
/// size
...
pub fn decode_for_base(
  ...
  match base {
    Base::Hex => {
      move |input: &[u8], output: &mut [u8]| {
        HEXLOWER_PERMISSIVE.decode_mut(input, output)
      }
    },
    Base::NixBase32 => {
      move |input: &[u8], output: &mut [u8]| base32::decode_mut(input, output)
    },
    Base::Base64 => {
      move |input: &[u8], output: &mut [u8]| BASE64.decode_mut(input, output)
    },
  }
}
...
pub fn encode_for_base(base: Base) -> impl Fn(&[u8], &mut [u8]) + 'static {
  match base {
    Base::Hex => {
      move |input: &[u8], output: &mut [u8]| {
        HEXLOWER_PERMISSIVE.encode_mut(input, output)
      }
    },
    Base::NixBase32 => {
      move |input: &[u8], output: &mut [u8]| base32::encode_mut(input, output)
    },
    Base::Base64 => {
      move |input: &[u8], output: &mut [u8]| BASE64.encode_mut(input, output)
    },
  }
}
